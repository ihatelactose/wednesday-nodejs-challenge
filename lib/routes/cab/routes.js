...
module.exports = [
    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            const driverExists = await server.methods.findOneDriver(driverId);
            if (!driverExists) {
                return h
                    .response({
    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            const response = nearby[0];
            const updatedReponse = response.map(cab => {
                const obj = {};
                obj.first_name = cab.first_name;
                obj.distance_m = Number.parseFloat(cab.distance_m).toFixed();
                return obj;
            });

            return h.response({ cabsNearby: updatedReponse });
        }
    },
    {
        method: 'PUT',
